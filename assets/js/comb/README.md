...
| `filter`            | string  | null          | CSS selector of the element to add filters to. When set, filtering is enabled. |
| `filterFields`      | object  | {}            | An object of fields to filter by, where keys are field names (based on the `data-*` attribute) and values are objects with the following key/value pairs: <ul><li>`text`: The text to display in the filter dropdown (defaults to the parent object's key, capitalised)</li></ul> |
